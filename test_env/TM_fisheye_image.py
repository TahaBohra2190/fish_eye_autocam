...
DISCLAIMER : In order to achieve an output user needs to alter values for 3 variables
            1) In line 14 for the object named test_image alter the path to master image for your device
...
sample_test_image = cv2.imread("images\\rawimage3.png",cv2.IMREAD_COLOR)

#funtion to extract the square from test image and store it to a png
def extract_target(test_image):
  ...
  # image_preview = cv2.resize(test_image, (1280,720))
  ...
  #You can add any required testing modules to this array and the for loop will also test for the added module
  method_s = [cv2.TM_SQDIFF ]

  for method in method_s:
    src2 = test_image.copy()
    ...
    #minMaxLoc goes through the matrix and find the minimum and the maximum value. Note that the input image does not neccesasary have to be grayscale.
  ...
  # Calculate and print the offset if needed
  ...
  region_of_interest = test_image[ location[1] : location[1] + W , location[0] : location[0] + H  ]
...
# a , b = extract_target(sample_test_image)
# print("Top left :",a," Bottom Right;",b)
...
#UNTESTED CODE TO BE INCLUDED IF NEEDED
